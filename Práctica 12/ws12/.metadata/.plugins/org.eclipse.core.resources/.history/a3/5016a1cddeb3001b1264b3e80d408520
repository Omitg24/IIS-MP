...
import uo.mp.marker.comparators.StudentComparatorExams;
import uo.mp.marker.comparators.StudentComparatorMark;
...
import uo.mp2021.file.ZipFileUtil;
...
public class ExamMarker{
	...
	public List<StudentMark> getMarksByStudent() {		
		Collections.sort(marks, new StudentComparatorMark());		
		return marks;
	}
	...
	/**
	 * Método que ordena los examenes por estudiante
	 * 
	 * @return studentExams, lista ordenada
	 */
	public List<StudentExam> getExamsByUO(){
		Collections.sort(studentExams, new StudentComparatorExams());	
		return studentExams;
	}
	...
	public void mark() {
		...
		for(StudentExam exam : studentExams) {
			...
			marks.add(new StudentMark(exam.getStudent(), total));
		}
	}
