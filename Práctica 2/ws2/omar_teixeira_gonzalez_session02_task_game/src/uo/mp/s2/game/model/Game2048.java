package uo.mp.s2.game.model;

import java.util.Random;

import uo.mp.util.checks.ArgumentsCheck;

/**
 * <p>
 * Título: Clase Game2048
 * </p>
 * <p>
 * Descripción: A partir de un array bidimensional de números y a través de
 * diferentes operaciones se simula un juego llamado "2048"
 * </p>
 * <p>
 * Copyright: Copyright (c) 2021
 * </p>
 * <p>
 * Empresa: Escuela de Ingeniería Informática - Uiversidad de Oviedo
 * </p>
 * 
 * @author Omar Teixeira González
 * @version 11/02/2021
 */
public class Game2048 {
	
	//Constantes:
	/**
	 * Dimensión minima necesaria de la matriz
	 */
	public static final int MIN_DIMENSION=3;
	
	/**
	 * Dimensión máxima necesaria de la matriz
	 */
	public static final int MAX_DIMENSION=10;	
	
	//Atributos:
	/**
	 * Atributo board
	 */
	private int[][] board;
	
	//Constructores:
	/**
	 * Constructor sin parámetros con tablero por defecto de 3*3 Inicialmente todas las posiciones con valor 0
	 */
	public Game2048() {
		board = new int[3][3];
		for (int i=0; i<board.length; i++) {
			for (int j=0; j<board[i].length; j++) {
				board[i][j]=0;
			}			
		}
	}

	/**
	 * Constructor que recibe la dimensión del tablero y, tras comprobar su validez, llama al metodo setBoardByDimension
	 *
	 * @param	dimension, dimension del tablero, de tipo int
	 */
	public Game2048(int dimension) {
		if (dimension >= MIN_DIMENSION && dimension <=MAX_DIMENSION) {
			setBoardByDimension(dimension);
		}else {
			board = new int[3][3];
			for (int i=0; i<board.length; i++) {
				for (int j=0; j<board[i].length; j++) {
					board[i][j]=0;
				}			
			}
		}
	}
	
	/**
	 * Constructor con parámetro board que, tras comprobar su validez, llama al método setBoardByBoard  
	 * 
	 * @param	board, tablero, de tipo int[][]
	 */
	public Game2048(int[][] board) {
		ArgumentsCheck.isTrue(board!=null && board.length>=MIN_DIMENSION && board.length<=MAX_DIMENSION && board[0].length>=MIN_DIMENSION 
				&& board[0].length<=MAX_DIMENSION && board.length==board[board.length-1].length, "El tablero no cumple los requisitos");
		setBoardByBoard(board);
	}

	//Métodos:
	/**
	 * Devuelve una copia de la matriz para poder usarla en las pruebas
	 * 
	 * @return	matrix, matriz copia de la matriz board
	 */
	public int[][] getBoard() {
		int[][] matrix = new int[board.length][board.length];
		for (int i=0; i<board.length; i++) {
			for (int j=0; j<board[i].length; j++) {
				matrix[i][j]=board[i][j];
			}
		}
		return matrix;
	}
	
	/**
	 * Método que modifica el valor del atributo board, 
	 * introduciendo como parámetro la dimensión del tablero a crear. 
	 * El tablero creado se inicia con un valor de todo ceros
	 * 
	 * @param dimension, dimension de la matriz a crear, de tipo int
	 */
	public void setBoardByDimension(int dimension) {
		board = new int[dimension][dimension];
		for (int i=0; i<board.length; i++) {
			for (int j=0; j<board[i].length; j++) {
				board[i][j]=0;
			}			
		}
	}	
	
	/**
	 * Método que modifica el valor del atributo board introduciendo como parámetro el tablero. 
	 * El tablero creado se inicia con un valor de todo ceros
	 * 
	 * @param board, tablero introducido por teclado, de tipo int[][]
	 */
	public void setBoardByBoard(int[][] board) {
		this.board = board;
	}	
	
	/**
	 * Reinicia todas las posiciones a 0 y llama a next para que incluya un 2 en una posición aleatoria
	 */
	public void restart() {
		for (int i=0; i<board.length; i++) {
			for (int j=0; j<board[i].length; j++) {
				board[i][j]=0;
			}
		}
		next();
	}
		
	/**
	 * Añade un nuevo número 2 en una posición aleatoria
	 */
	public void next() {
		Random random=new Random();
		int randomRow=random.nextInt(board.length);
		int randomColumn=random.nextInt(board.length);
		while (board[randomRow][randomColumn]!=0) {
			randomRow=random.nextInt(board.length);
			randomColumn=random.nextInt(board.length);
		}
		board[randomRow][randomColumn]=2;
	}
	
	/**
	 * Comprueba si el tablero está lleno. Esto ocurre cuando todas las celdas o posiciones del tablero tienen un número distinto de cero
	 * 
	 * @return true si la matriz esta llena y false si no lo está
	 */
	public boolean isBoardFull() {
		for (int i=0; i<board.length; i++) {
			for (int j=0;j<board[i].length;j++) {
				if (board[i][j]==0){
					return false;
				}
			}
		}
		return true;		
	}

	/**
	 * Compacta el tablero a la derecha compactando todas las filas
	 */
	public void compactRight() {
		for (int x=0; x<board.length;x++) {
			for (int y=board[x].length-2; y>=0;y--) {
				if (board[x][y]!=0) {
					int i=1;
					int j=0;
					while (y+i<board.length) {
						if (board[x][y+i]==0) {
						board[x][y+i]=board[x][y+j];
						board[x][y+j]=0;						
						}				
						i++;
						j++;
					}
				}					
			}
		}
	}
	
	/**
	 * Método que suma dos elementos consecutivos a la derecha del tablero y 
	 * elimina el elemento que estaba una posicion a la izquierda del sumado.
	 * Llama al método compactRight
	 */
	public void moveRight() {
		compactRight();
		for (int i=0; i<board.length;i++) {
			for (int j=board.length-2; j>=0;j--) {
				if (board[i][j]==board[i][j+1]) {
					board[i][j+1]=board[i][j]+board[i][j+1];
					board[i][j]=0;
				}
			}
		}
		compactRight();
	}	
	
	/**
	 * Compacta el tablero a la izquierda compactando todas las filas
	 */
	public void compactLeft() {
		for (int x=0; x<board.length;x++) {
			for (int y=1; y<board[x].length;y++) {
				if (board[x][y]!=0) {
					int i=1;
					int j=0;
					while (y-i>=0) {
						if (board[x][y-i]==0) {
						board[x][y-i]=board[x][y-j];
						board[x][y-j]=0;						
						}				
						i++;
						j++;
					}
				}	
			}
		}
	}
	
//	Método moveLeft mediante el rotateBoard:
	/**
	 * Método que suma dos elementos consecutivos a la izquierda del tablero y 
	 * elimina el elemento que estaba una posicion a la derecha del sumado.
	 * Llama al método rotateBoard y moveRight 
	 */
	public void moveLeft() {
		rotateBoard();
		rotateBoard();
		moveRight();
		rotateBoard();
		rotateBoard();
	}
	
	/**
	 * Método que suma dos elementos consecutivos a la izquierda del tablero y 
	 * elimina el elemento que estaba una posicion a la derecha del sumado.
	 * Llama al método compactLeft
	 */
	public void moveLeftSecondOption() {
		compactLeft();
		for (int i=0; i<board.length;i++) {
			for (int j=0; j<board.length-1;j++) {
				if (board[i][j]==board[i][j+1]) {
					board[i][j]=board[i][j]+board[i][j+1];
					board[i][j+1]=0;
				}
			}
		}
		compactLeft();
	}

	/**
	 * compacta toda la matriz hacia arriba compactando todas las filas
	 */
	public void compactUp() {
		for (int x=0; x<board.length;x++) {
			for (int y=1; y<board[x].length;y++) {
				if (board[y][x]!=0) {
					int i=1;
					int j=0;
					while (y-i>=0) {
						if (board[y-i][x]==0) {
						board[y-i][x]=board[y-j][x];
						board[y-j][x]=0;						
						}				
						i++;
						j++;
					}
				}	
			}
		}
	}
	
	
//	Método moveUp mediante el rotateBoard:
	/**
	* Método que suma dos elementos consecutivos superiormente del tablero y 
	 * elimina el elemento que estaba una posicion por debajo.
	 * Llama al método rotateBoard y moveRight 
	 */
	public void moveUp() {
		rotateBoard();
		rotateBoard();		
		rotateBoard();
		moveRight();
		rotateBoard();
	}
	
	/**
	 * Método que suma dos elementos consecutivos superiormente del tablero y 
	 * elimina el elemento que estaba una posicion por debajo.
	 * Llama al método compactUp
	 */
	public void moveUpSecondOption() {
		compactUp();
		for (int i=0; i<board.length-1;i++) {
			for (int j=0; j<board.length;j++) {
				if (board[i][j]==board[i+1][j]) {
					board[i][j]=board[i][j]+board[i+1][j];
					board[i+1][j]=0;
				}
			}
		}
		compactUp();
	}

	/**
	 * Compacta la matriz hacia abajo compactando todas las filas
	 */
	public void compactDown() {
		for (int x=0; x<board.length;x++) {
			for (int y=board[x].length-2; y>=0;y--) {
				if (board[y][x]!=0) {
					int i=1;
					int j=0;
					while (y+i<board.length) {
						if (board[y+i][x]==0) {
						board[y+i][x]=board[y+j][x];
						board[y+j][x]=0;						
						}				
						i++;
						j++;
					}
				}					
			}
		}
	}
	
//	Método moveDown mediante el rotateBoard:
	/**
	 * Método que suma dos elementos consecutivos inferiormente del tablero y 
	 * elimina el elemento que estaba una posicion por encima.
	 * Llama al método rotateBoard y moveRight 
	 */
	public void moveDown() {
		rotateBoard();
		moveRight();
		rotateBoard();		
		rotateBoard();				
		rotateBoard();
	}
	
	/**
	 * Método que suma dos elementos consecutivos inferiormente del tablero y 
	 * elimina el elemento que estaba una posicion por encima.
	 * Llama al método compactDown
	 */
	public void moveDownSecondOption() {
		compactDown();
		for (int i=board.length-2; i>=0;i--) {
			for (int j=0; j<board.length;j++) {
				if (board[i][j]==board[i+1][j]) {
					board[i][j]=board[i][j]+board[i+1][j];
					board[i+1][j]=0;
				}
			}
		}
		compactDown();
	}	
			
	/**
	 * Devuelve la matriz en formato para ser mostrado por pantalla
	 * 
	 * @return	matrix, la matriz en tipo String 
	 */
	public String toString() {
		String matrix="";
		for (int i=0; i<board.length;i++) {
			for (int j=0; j<board[i].length;j++) {
				matrix+="  " + board[i][j] + "  ";
			}
			matrix += "\n";
		}
		return matrix;				
	}
	
	/**
	 * Método que rota la matriz en sentido antihorario 
	 */
	public void rotateBoard() {
		for(int i = 0; i < board.length/2; i++){
			for(int j = i; j < board.length-i-1; j++){
				int aux = board[j][i];
				board[j][i] = board[i][board.length-j-1];
				board[i][board.length-j-1] = board[board.length-j-1][board.length-i-1];
				board[board.length-j-1][board.length-i-1] = board[board.length-i-1][j];
				board[board.length-i-1][j] = aux;
			}
		}
	}
}
