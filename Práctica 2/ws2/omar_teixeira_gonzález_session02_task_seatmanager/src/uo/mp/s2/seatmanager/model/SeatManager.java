    ...
    */
    public ArrayList<Person> findPassengersByAge(int min_age, int max_age){
    	ArgumentsCheck.isTrue(min_age>=Person.MIN_AGE && min_age<=Person.MAX_AGE, 
    			"El límite inferior (edad mínima) no cumple los parámetros");
    	ArgumentsCheck.isTrue(max_age>=Person.MIN_AGE && max_age<=Person.MAX_AGE, 
    			"El límite superior (edad máxima) no cumple los parámetros");
    	ArgumentsCheck.isTrue(min_age<max_age, "El limite mínimo es mayor que el máximo");
...
//Método extra
    /**
     * Método que convierte el array bidimensional a String con una identificación en función de la edad
     */
